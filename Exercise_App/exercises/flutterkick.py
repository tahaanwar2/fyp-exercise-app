...
class Flutterkick():
    ...
    def start_exercise(self , mp_drawing , mp_pose, col, video_path):
        ...
        with col: #Horizontal Aline
            run = st.checkbox('FlutterKick', key='flutterkick')
            ...
            while cap.isOpened() and run:
                ...
                if ret:
                    ...
                    try:
                        ...
                        parallel_angle = calculate_angle(right_ankle, right_hip, left_ankle)
                        ...
                        #             tuple(np.multiply(left_ankle, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        # cv2.putText(self.image, "Right Angle: " + str(right_angle), 
                        #             tuple(np.multiply(right_ankle, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        
                        # cv2.putText(self.image, "Parallel Angle: " + str(parallel_angle), 
                        ...
                        if left_position_angle<30 and right_position_angle<30:
                            if left_angle>160 and right_angle>160 and parallel_angle>55:
                                self.up = True
                            if parallel_angle<20 and self.up:
                                self.up = False
                                self.count+=1
                                self.calories_burned += calorie_counter("flutterkick") 
                        else:
