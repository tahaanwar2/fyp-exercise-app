...
class Crunches():
    def __init__(self) :
        ...
        self.left_right = False #Left=False, Right=True
    ...
    def position(self, left_position_angle, right_position_angle):
        if int(left_position_angle) in range(25, 60) and int(right_position_angle) in range(25, 60):
    ...
    def draw_position_text(self):
        cv2.putText(self.image, str('Please set your position'), 
            (150,50), 
            cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 0 , 0), 2, cv2.LINE_AA
            )
    ...
    def draw_counter(self):
        if self.init_time is not None:
            if (time.time() > self.counter_timeout_text and time.time() < self.test_timeout):
                cv2.putText(self.image  , 'Timer : ' + str(self.counter) , (700 , 30) ,cv2.FONT_HERSHEY_SIMPLEX , 1, (0, 247, 255),2 , cv2.LINE_AA )
    ...
    def start_exercise(self, mp_drawing , mp_pose, col, video_path):
        ...
        with col: #Horizontal Aline
            run = st.checkbox('Crunches', key='crunches')
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            ...
            while cap.isOpened() and run:
                ...
                if ret:
                    ...
                    try:
                        ...
                        left_position_angle = calculate_angle(left_hip, left_knee, left_ankle)
                        right_position_angle = calculate_angle(right_hip, right_knee, right_ankle)
                        ...
                        if not self.position_check:
                            self.position(left_position_angle , right_position_angle)
                            self.draw_position_text()
                            continue
                        else:
                            self.initialize_time_counter()

                        # Visualize angle
                        # cv2.putText(self.image, str(left_angle), 
                        #             tuple(np.multiply(left_elbow, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        # cv2.putText(self.image, str(right_angle), 
                        #             tuple(np.multiply(right_elbow, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        
                        # cv2.putText(self.image, str(left_position_angle), 
                        #             tuple(np.multiply(left_knee, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        # cv2.putText(self.image, str(right_position_angle), 
                        #             tuple(np.multiply(right_knee, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )



                        if left_angle<80 and right_angle>80 and self.left_right==False:
                            self.left_right = True
                            self.count+=1
                            self.calories_burned += calorie_counter("crunches")                          
                        
                        if right_angle<80 and self.left_right==True:
                    ...
                    try:
                        FRAME_WINDOW.image(cv2.cvtColor(self.image, cv2.COLOR_BGR2RGB))
                    except:
                        FRAME_WINDOW.empty()
                        break
                else:
            ...
            if self.count>0:
                ...
                with output_col2:
                    st.metric(label="Caloried Burned", value=str(self.calories_burned)[:5])
            # cv2.destroyAllWindows()