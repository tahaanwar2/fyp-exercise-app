...
class Jumping():
    ...
    def start_exercise(self , mp_drawing , mp_pose, col, video_path):
        ...
        with col: #Horizontal Aline
            run = st.checkbox('Jumping', key='jumping')
            ...
            while cap.isOpened() and run:
                ...
                if ret:
                    ...
                    try:
                        ...
                        # Calculate angle
                        left_angle = calculate_angle(left_wrist, left_shoulder, right_wrist)
                        right_angle = calculate_angle(right_wrist, right_shoulder, left_wrist)
                        
                        left_position_angle = calculate_angle(left_wrist, left_shoulder, left_hip)
                        right_position_angle = calculate_angle(right_wrist, right_shoulder, right_hip)
                    
                        if not self.callibration_check:
                            self.callibration(left_hip , left_shoulder)
                        ...
                        # Visualize angle
                        # cv2.putText(self.image, "Left Angle: " + str(left_angle), 
                        #             tuple(np.multiply(left_wrist, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        # cv2.putText(self.image, "Right Angle: " + str(right_angle), 
                        #             tuple(np.multiply(right_wrist, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        
                        # cv2.putText(self.image, "Left Position: " + str(left_position_angle), 
                        #             tuple(np.multiply(left_shoulder, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )
                        # cv2.putText(self.image, "Right Position: " + str(right_position_angle), 
                        #             tuple(np.multiply(right_hip, [h, w]).astype(int)), 
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA
                        #                     )

                        if left_angle<20 and right_angle<20 and left_position_angle>100 and right_position_angle>100:
                            self.up = True
                        if left_angle>50 and right_angle>50 and left_position_angle>100 and right_position_angle>100 and self.up:
                            self.up = False
                            self.count+=1
                            self.calories_burned += calorie_counter("jumping")                          
