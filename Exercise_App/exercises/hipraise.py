...
class Hipraise():
    ...
    def start_exercise(self, mp_drawing, mp_pose, col, video_path):
        ...
        with col:  # Horizontal Aline
            run = st.checkbox('HipRaise', key='hipraise')
            ...
            while cap.isOpened() and run:
                ...
                if ret:
                    ...
                    try:
                        ...
                        left_wrist_position_angle = calculate_angle(
                            left_hip, left_shoulder, left_wrist)
                        right_wrist_position_angle = calculate_angle(
                        ...
                        # cv2.putText(self.image, "Left Wrist Position: " + str(left_wrist_position_angle),
                        #             tuple(np.multiply(
                        #                 left_ankle, [h, w]).astype(int)),
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (
                        #                 0, 0, 255), 2, cv2.LINE_AA
                        #             )
                        # cv2.putText(self.image, "Right Wrist Position: " + str(right_wrist_position_angle),
                        #             tuple(np.multiply(
                        #                 right_hip, [h, w]).astype(int)),
                        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (
                        #                 0, 0, 255), 2, cv2.LINE_AA
                        ...
                        if left_position_angle < 130 and right_position_angle < 130 and left_angle > 90 and right_angle > 90 and (left_wrist_position_angle > 45 or right_wrist_position_angle > 45):
                            self.up = True
                        if left_position_angle > 130 and right_position_angle > 130 and (left_wrist_position_angle < 45 or right_wrist_position_angle < 45) and self.up:
                            self.up = False
                            self.count += 1
                            self.calories_burned += calorie_counter("hipraise")
